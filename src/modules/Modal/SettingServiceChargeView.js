...
export default class SettingServiceChargeView extends Component<Props> {
	...
	async clickSettingServiceCharge() {
		...
		if (response.success) {
			...
			GFunction.successMessage(I18n.t('message.success'), I18n.t('message.settingServiceChargeSuccessful'));
	...
	render() {
		return (
			<View style={{flex: 1, padding: 30}}>
				<Text style={{fontSize: 30}}>{I18n.t('placeholder.settingServiceCharge')}</Text>
				<View style={{paddingTop: 15}}>
					...
					/>
					<HelperText type="error" visible={GFunction.validateBlank(this.state.serviceCharge)}>
