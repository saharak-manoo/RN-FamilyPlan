...
export default class InviteMemberView extends Component<Props> {
	...
	async clickInviteMember() {
		...
		if (response.success) {
			...
			if (response.user != null) {
				...
				GFunction.infoMessage(I18n.t('message.notFound'), I18n.t('message.notFoundUser'));
			}
		} else {
			this.loadingInviteMember.showLoading(false);
			let errors = [];
	...
	async addToGroup(id) {
		...
		if (response.success) {
			...
			GFunction.successMessage(I18n.t('message.success'), I18n.t('message.inviteMemberSuccessful'));
			...
			<View style={{flex: 1, padding: 30}}>
				...
				<View style={{paddingTop: 15}}>
					<TextInput
						...
						onChangeText={newMemberEmail => this.setState({newMemberEmail: newMemberEmail})}
					/>
					<HelperText type="error" visible={GFunction.validateEmail(this.state.newMemberEmail)}>
				...
				<View style={{paddingTop: 15}}>
					{this.state.newMember != null ? (
						<ListItem
							...
							containerStyle={{backgroundColor: '#F5F5F5', borderRadius: 20}}
							chevron={
								<MatIcon
									name="add"
									size={35}
