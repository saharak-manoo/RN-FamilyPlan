...
export default class SetUpReminderView extends Component<Props> {
	...
	async clickSettingDueDate() {
		...
		if (response.success) {
			...
			GFunction.successMessage(I18n.t('message.success'), I18n.t('message.settingDueDateSuccessful'));
	...
	render() {
		return (
			<View style={{flex: 1, padding: 30}}>
				<Text style={{fontSize: 30}}>{I18n.t('placeholder.setUpAReminder')}</Text>
				<View style={{paddingTop: 15}}>
					<DatePicker date={this.state.dueDate} mode="date" onDateChange={dueDate => this.setState({dueDate})} />
				</View>
