...
import {Dimensions, FlatList, Platform, Modal, RefreshControl, ScrollView, TouchableOpacity, StatusBar, View} from 'react-native';
...
export default class HomeView extends Component<Props> {
							...
							<View style={{flex: 1}}>
								...
								<View style={{flex: 0.3}}>
									<Text numberOfLines={1} style={styles.totalMembersCard}>
										{I18n.t('placeholder.members')} : {item.members.length}/{item.max_member}
									</Text>
								</View>
							</View>
						</TouchableOpacity>
	...
	listPublicGroup = publicGroup => {
		return (
			<FlatList
				style={{flex: 1}}
				data={publicGroup.filter(group => group.members.length < group.max_member)}
				horizontal={true}
				showsHorizontalScrollIndicator={false}
				renderItem={({item, index}) => {
					return (
	...
	render() {
		return (
			<View style={styles.defaultView}>
				...
				{this.state.spinner ? (
					<Spinner visible={this.state.spinner} textContent={I18n.t('placeholder.loading') + '...'} textStyle={styles.spinnerTextStyle} />
				) : (
					<ScrollView refreshControl={<RefreshControl refreshing={this.state.refreshing} onRefresh={this.refreshGroup} />}>
						<View style={{flex: 1, padding: 15, paddingTop: 35}}>
							<View style={{flex: 1}}>
								...
								<View style={styles.listCards}>
									{this.state.myGroups.length !== 0 ? (
										...
										<View>
											<TouchableOpacity style={styles.card} onPress={this.showNewGroupModal}>
												<View style={{flex: 1}}>
													<View style={styles.headerCardNewGroup}>
														<Icon reverse name="add" type="mat-icon" color="#00C15E" />
													</View>
													<View style={{flex: 0.4}}>
														<Text
															numberOfLines={1}
							...
							{this.state.publicGroups.length !== 0 ? (
								<View style={{flex: 1, paddingTop: 40}}>
									...
									<View style={styles.listCards}>{this.listPublicGroup(this.state.publicGroups)}</View>
								</View>
							) : null}
						</View>
					</ScrollView>
